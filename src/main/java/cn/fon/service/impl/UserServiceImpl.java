...
//            (required = false)
    ...
    @Override
    public String selectByPage(Integer offset, Integer limit, String name, String telephone, String address) {
        if (!StringUtils.isEmpty(name) || !StringUtils.isEmpty(telephone) || !StringUtils.isEmpty(address)) {
            // 如果有条件，则将分页设置从0开始，避免从非第一页关键字查询发起请求
            offset = 0;
            limit = 10;
        }
        System.out.println("测试:进入到用户查询方法selectByPage");
        return JSON.toJSONString(new BootstrapTable(userMapper.selectCount(null), userMapper.selectByPage(offset, limit, name, telephone, address)));
